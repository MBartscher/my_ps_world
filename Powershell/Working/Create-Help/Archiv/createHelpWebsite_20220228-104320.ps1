...
#--------Fuctions----------------------------------------------------------------------------------------------------------------#
#--------------------------------------------------------------------------------------------------------------------------------#
function MakeSure-ThatFolderExists{
 param(
  [Parameter(Mandatory=$true)][string]$strParentPath,
...
function MakeSure-ThatPathExists{
 param([Parameter(Mandatory=$true)][string]$strPath)
 [string[]]$strPathParts=$strPath.Split('\')
 [string]$strPartialPath=$strPathParts[0]
 for($i=1;$i-lt$strPathParts.Count;$i++){
  MakeSure-ThatFolderExists -strParentPath $strPartialPath -strName $strPathParts[$i]
...
function Create-HTMLFile{
 param(
  ...
  [Parameter(Mandatory=$true)][string]$strBody
 )
 if(Test-Path(-join($strPath,'\',$strName,'.htm'))){Remove-Item -Path (-join($strPath,'\',$strName,'.htm')) -Force}
 New-Item -Path $strPath -Force -ItemType File -Name (-join($strName,'.htm')) -Value ([string](ConvertTo-Html -Title $strTitle -Body $strBody))
 $strPath=$null
 $strName=$null
 $strTitle=$null
 $strBody=$null
...
function Create-ContentFromTxtFile{
 ...
 [string[]]$astrContentRaw=Get-Content(-join($strTxtPath,'\',$strName,'.txt'))
 ...
 for($i=0;$i-lt$astrContentRaw.Count;$i++){
  $intRem=Show-PercentPoints -intCount $i -intMax $astrContentRaw.Count -intOld $intRem
...
#--------------------------------------------------------------------------------------------------------------------------------#
Clear-Host
MakeSure-ThatPathExists -strPath $strFolderPath
#--------CreateHelpTxtFiles--------#
#CreateHelpTxtFiles
...
if($createNewTxtFiles.IsPresent-or$bTxtFilesMissing){
 ...
 MakeSure-ThatFolderExists -strParentPath $strFolderPath -strName 'txt'
 if($updateHelp.IsPresent){C:\DASI\Skripte\Powershell\Working\createHelp\createHelpComplet_20220226-104012.ps1 -strFolderPath (($strFolderPath,'txt')-join'\') -updateHelp}
 else{C:\DASI\Skripte\Powershell\Working\createHelp\createHelpComplet_20220226-104012.ps1 -strFolderPath (($strFolderPath,'txt')-join'\')}
}
#--------CreateSortedLists&SitesFromSortedLists--------#
Write-Host '#=> create `$astrAll'
[string[]]$astrAll=$null
Get-ChildItem(($strFolderPath,'txt')-join'\')|Sort-Object -Property Name|foreach-object{$astrAll+=$_.Name.replace('.txt','')}
 ...
 elseif($astrAll[$i].contains('-')){
  ...
  if($astrNouns-ne$null){
   ...
   for($k=0;$k-lt$astrNouns.Count;$k++){
    $intRem=Show-PercentPoints -intCount $k -intMax $astrNouns.Count -intOld $intRem
...
for($i=1;$i-lt$astrNouns.Count;$i++){
 $intRem=Show-PercentPoints -intCount $i -intMax $astrNouns.Count -intOld $intRem
 $bSwapped=$false
