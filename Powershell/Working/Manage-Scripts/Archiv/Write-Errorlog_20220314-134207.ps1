...
process{
 if($Error.Count-gt0){
  ...
  while($Error.Count-gt0){
   if(((($Error[0].ToString()-cnotmatch$strExclude)-and$strExclude)-or(($Error[0].ToString()-cmatch$strInclude)-and$strInclude)-or((-not$strExclude)-and(-not$strInclude)))-and(-not($Error[0]-is[System.Management.Automation.TerminateException]))){
    ...
    if($Error[0]-is[System.Management.Automation.ErrorRecord]){Export-Clixml -Path (-join('C:\DASI\Skripte\Powershell\ErrorLogs\',$strFileDateTimeUniversal,'.xml')) -Force -Depth 5 -InputObject $Error[0]}
 ...
 else{Remove-Item -Path C:\DASI\Skripte\Powershell\ErrorLogs -Force}
 }
}
end{
