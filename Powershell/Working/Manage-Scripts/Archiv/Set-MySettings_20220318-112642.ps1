...
begin{
 ...
 if(-not($strMySettingsScriptName)){[string](New-Variable -Name strMySettingsScriptName -Value 'MySettings.ps1' -Option AllScope,ReadOnly -Force -Scope Global)}
...
process{
 ...
 $strScriptMySettings+=-join('[string](New-Variable -Name strMySettingsScriptName -Value ''',$strMySettingsScriptName,''' -Option AllScope,ReadOnly -Force -Scope Global)',$strBreak)
 ...
 if(Test-Path -Path (-join($strPathScripts,$strMySettingsScriptName))){Remove-Item -Path (-join($strPathScripts,$strMySettingsScriptName)) -Force}
 if($VerbosePreference){New-Item -Path $strPathScripts -Name $strMySettingsScriptName -ItemType File -Value $strScriptMySettings -Force}
 else{New-Item -Path $strPathScripts -Name $strMySettingsScriptName -ItemType File -Value $strScriptMySettings -Force|Out-Null}
}
end{
