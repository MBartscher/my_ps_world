...
begin{
 ...
 if(-not(Test-Path -Path (-join('Alias:',$Matches[0])))){C:\DASI\Skripte\Powershell\Working\Manage-Files\Set-Aliases_20220303-132221.ps1 -strFolderPath ($MyInvocation.MyCommand.Path-replace($MyInvocation.MyCommand.Name,''))}
}
process{
 if($Error.Count-gt0){
