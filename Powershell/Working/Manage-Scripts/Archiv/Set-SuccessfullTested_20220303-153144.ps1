...
process{
 ...
 if($aflinfSameScriptDiffVers.Count-gt1){
  ...
  for($i=1;$i-lt$aflinfSameScriptDiffVers.Count;$i++){Move-Item -Path $aflinfSameScriptDiffVers[$i].FullName -Destination ($aflinfSameScriptDiffVers[$i].FullName-replace($aflinfSameScriptDiffVers[$i].Name,'')) -Force}
 }
