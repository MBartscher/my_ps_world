...
process{
 ...
 if(Test-Path variable:strFileDateTimeUniversal){Remove-Variable -Name strFileDateTimeUniversal -Force}
 if(Test-Path variable:iExcluded){Remove-Variable -Name iExcluded -Force}
 if(Test-Path variable:strExclude){Remove-Variable -Name strExclude -Force}
 if(Test-Path variable:strInclude){Remove-Variable -Name strInclude -Force}
 if(Test-Path variable:OpenErrorLogsFolder){Remove-Variable -Name OpenErrorLogsFolder -Force}
 if(Test-Path variable:strPathErrorLogs){Remove-Variable -Name strPathErrorLogs -Force}
 if(Test-Path variable:iCountOld){Remove-Variable -Name iCountOld -Force}
}